...
class Tetris:
    ...
    def render(self, video=None):
        ...
        # 新增：绘制黑色粗边框
        # ==========================================
        border_color = (0, 0, 0) # 黑色 (BGR)
        border_thickness = 8     # 边框粗细
        ...
        cv2.putText(img, "Score:", (text_x, self.block_size),
                    fontFace=font, fontScale=0.7, color=self.text_color, thickness=1, lineType=cv2.LINE_AA)
        cv2.putText(img, str(self.score), (text_x, 2 * self.block_size),
                    fontFace=font, fontScale=0.7, color=self.text_color, thickness=1, lineType=cv2.LINE_AA)

        cv2.putText(img, "Lines:", (text_x, 4 * self.block_size),
                    fontFace=font, fontScale=0.7, color=self.text_color, thickness=1, lineType=cv2.LINE_AA)
        cv2.putText(img, str(self.cleared_lines), (text_x, 5 * self.block_size),
                    fontFace=font, fontScale=0.7, color=self.text_color, thickness=1, lineType=cv2.LINE_AA)
